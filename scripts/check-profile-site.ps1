...
$themes = Read-Source '_sass/_themes.scss'
$aboutLayout = Read-Source '_layouts/about.liquid'
$pageLayout = Read-Source '_layouts/page.liquid'
$cvLayout = Read-Source '_layouts/cv.liquid'
$bibLayout = Read-Source '_layouts/bib.liquid'
$projectInclude = Read-Source '_includes/projects.liquid'
$cvTimeTable = Read-Source '_includes/cv/time_table.liquid'
...
Require-Match $bibLayout 'publication-index' 'Publication index markers are missing.'
Require-Match $projectInclude 'project-index' 'Project index labels are missing.'
Require-Match $themes '--global-editorial-accent' 'Editorial theme tokens are missing.'
Require-Match $styles 'prefers-reduced-motion' 'Reduced-motion handling is missing.'
Require-Match $cvTimeTable 'cv-date-column' 'CV dates do not have a responsive date column.'
Require-Match $cvTimeTable 'cv-entry-content' 'CV entries do not have a flexible content column.'
Require-NoMatch $cvTimeTable 'style="width:\s*75px"' 'CV date columns still use a fixed 75px inline width.'
Require-NoMatch $cvTimeTable 'style="min-width:\s*75px"' 'CV date labels still use a fixed 75px inline width.'
...
Require-Match $styles '\.cv\s+\.card\s+\.list-group-item\s+\.badge' 'The CV date override is not specific enough to beat the theme badge rule.'
